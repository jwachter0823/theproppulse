...
const NavBar = ({tab,setTab,setPage,page,user,onLogin,onLogout,setPpSection,navRefresh}) => {
  ...
  useEffect(()=>{
    if(!user) return;
    supabase.from("profiles").select("points,avatar_url").eq("id",user.id).single().then(({data})=>{if(data){setUserPts(data.points||0);setAvatarUrl(data.avatar_url||null)}});
  },[user,tab,page,navRefresh]);
  ...
  return (<>
    <nav className="nav">
      ...
      {user?<div className="nav-user">
        <div className="nav-avatar" onClick={()=>setShowProfile(p=>!p)} style={avatarUrl?{padding:0,overflow:'hidden'}:{}}>{avatarUrl?<img src={avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:(user.email||"U")[0].toUpperCase()}</div>
      </div>
    ...
    {showProfile&&user&&<>
      ...
      <div className="panel">
        ...
        <div className="panel-header">
          <div className="panel-avatar" style={avatarUrl?{padding:0,overflow:'hidden'}:{}}>{avatarUrl?<img src={avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:(user.email||"U")[0].toUpperCase()}</div>
...
export default function App() {
  ...
  return (<><style>{css}</style>
    ...
    <div className="page">
    <Ticker/>
    <NavBar tab={tab} setTab={setTab} setPage={setPage} page={page} user={user} onLogin={()=>setShowAuth(true)} onLogout={handleLogout} setPpSection={setPpSection} navRefresh={navRefresh}/>
    <div className="wrap">
      <div className="hero">
    ...
    <div className="wrap">
      ...
      {tab==="points"&&<PulsePointsTab user={user} onLogin={()=>setShowAuth(true)} ppSection={ppSection} setPpSection={setPpSection} navRefresh={navRefresh}/>}
    </div>
