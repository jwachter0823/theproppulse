...
export default function App() {
  ...
  if(page==="account") return (<><style>{css}</style><div className="ambient"><div className="ambient-gold"/></div><div className="edge-glow"/><div className="side-glow-l"/><div className="side-glow-r"/><div className="top-glow"/><div className="page"><Ticker/><NavBar tab={tab} setTab={setTab} setPage={setPage} page={page} user={user} onLogin={()=>setShowAuth(true)} onLogout={handleLogout} setPpSection={setPpSection} navRefresh={navRefresh} avatarUrl={avatarUrl}/><div className="wrap"><AccountPage user={user} goBack={()=>{setPage("home");window.scrollTo({top:0,behavior:"smooth"})}} onAvatarChange={()=>setNavRefresh(n=>n+1)}/></div><Footer setPage={setPage} setTab={setTab}/></div></>);
