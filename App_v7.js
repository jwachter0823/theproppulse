...
const NavBar = ({tab,setTab,setPage,page,user,onLogin,onLogout,setPpSection,navRefresh,avatarUrl}) => {
  ...
  const ava = avatarUrl || localAvatar;

  useEffect(()=>{
    if(!user) return;
    supabase.from("profiles").select("points,avatar_url").eq("id",user.id).single().then(({data})=>{if(data){setUserPts(data.points||0);if(data.avatar_url)setLocalAvatar(data.avatar_url)}});
  },[user,tab]);
...
const AccountPage = ({user,goBack,onAvatarChange}) => {
...
export default function App() {
  ...
  useEffect(()=>{
    if(!user) return;
    supabase.from("profiles").select("avatar_url").eq("id",user.id).single().then(({data})=>{if(data)setAvatarUrl(data.avatar_url||null)});
  },[user,navRefresh]);
  ...
  return (<><style>{css}</style>
    ...
    <div className="page">
    <Ticker/>
    <NavBar tab={tab} setTab={setTab} setPage={setPage} page={page} user={user} onLogin={()=>setShowAuth(true)} onLogout={handleLogout} setPpSection={setPpSection} navRefresh={navRefresh} avatarUrl={avatarUrl}/>
    <div className="wrap">
      <div className="hero">
    ...
    <div className="wrap">
      ...
      {tab==="points"&&<PulsePointsTab user={user} onLogin={()=>setShowAuth(true)} ppSection={ppSection} setPpSection={setPpSection} navRefresh={navRefresh} avatarUrl={avatarUrl}/>}
    </div>
